...
int SelecLvlGame()
{
    ...
    int MenuY = 0;
    int indexLvMenu = 1;
    MatrixWrite(menuLvGame);
    ConsoleKeyInfo User_keyTab = Console.ReadKey();
    while(User_keyTab.Key != ConsoleKey.Enter)
    {
    
        Console.Clear();
        menuLvGame[MenuY,MenuX] = " ";
        if(User_keyTab.Key == ConsoleKey.W && indexLvMenu > 0)
        {
            indexLvMenu--;
            MenuY--;
        }
        if(User_keyTab.Key == ConsoleKey.S && indexLvMenu < 5)
        {
           indexLvMenu++;
            MenuY++; 
        }
...
{   
    ...
    Console.WriteLine("Колличество очков " + coins );
}
